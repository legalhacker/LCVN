...
// ============================================================================
// LEGAL CITATION GRAPH — Phase 3 Graph-ready API (lookup by semantic_id)
// ============================================================================

// GET /api/articles/:semanticId/citation-network
// Returns the full citation graph node: outgoing + incoming edges
router.get('/:semanticId/citation-network', async (req, res: Response, next) => {
  try {
    ...
    const article = await prisma.article.findUnique({
      ...
      select: {
        ...
        legalTopics: true,
        document: {
          select: { documentNumber: true, title: true, titleSlug: true, documentType: true, issuedDate: true },
        },
      },
    });
    ...
    const edgeArticleSelect = {
      select: {
        ...
        title: true,
        document: {
          select: { documentNumber: true, title: true, titleSlug: true },
        },
      },
    } as const;

    const typeFilter = req.query.type ? { relationType: req.query.type as any } : {};

    const [outgoing, incoming] = await Promise.all([
      prisma.articleRelation.findMany({
        where: { fromArticleId: article.id, ...typeFilter },
        include: { toArticle: edgeArticleSelect },
        orderBy: { createdAt: 'asc' },
      }),
      prisma.articleRelation.findMany({
        where: { toArticleId: article.id, ...typeFilter },
        include: { fromArticle: edgeArticleSelect },
        orderBy: { createdAt: 'asc' },
      }),
    ]);
    ...
    res.json({
      ...
      outgoing: outgoing.map(r => ({
        edge_id: r.id,
        source_id: article.semanticId,
        target_id: r.toArticle.semanticId,
        relationship_type: r.relationType,
        notes: r.note ?? null,
        created_by: r.createdBy ?? null,
        created_at: r.createdAt,
        target_article: r.toArticle,
      })),
      incoming: incoming.map(r => ({
        edge_id: r.id,
        source_id: r.fromArticle.semanticId,
        target_id: article.semanticId,
        relationship_type: r.relationType,
        notes: r.note ?? null,
        created_by: r.createdBy ?? null,
        created_at: r.createdAt,
        source_article: r.fromArticle,
      })),
    });
  } catch (error) {
    next(error);
...
// GET /api/articles/:semanticId/legal-annotations
// Returns admin-curated legal annotations for an article node
// (distinct from user workspace annotations at /:articleId/annotations)
router.get('/:semanticId/legal-annotations', async (req, res: Response, next) => {
  try {
    const { semanticId } = req.params;
    const { type, visibility } = req.query;

    const where: any = { nodeId: semanticId };
    if (type) where.type = type as string;
    if (visibility) where.visibility = visibility as string;
    else where.visibility = 'public'; // public-facing: only return public by default
